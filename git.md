...
Чтобы при запуске ярлыка Git bash сразу открывалась рабочая директория:

-   в свойствах ярлыка, в поле *Рабочая папка*, устанавливаем необходимый путь

-   в поле *Объект* удаляем в конце строки `--cd-to-home`
...
Просмотреть переменные можно командой:
...
узнать статус репозитория (покажет ф-лы изменения в кот-х не были отправлены на
сервер):
...
**git add .** `#добавляет все сделанные изменения в тек. папке`
...
команда слияния (аналогична merge, только без порядка вноса изменений ‘истории
коммитов’):

**git rebase**

**rebase dev** `#слить с веткой dev`

Если индексы коммитов в локальном репозитории отличаются от удаленного, но мы
все-равно хотим заменить локальные данные, данными с удаленного сервера
используем команду “спрятать”:

**git stash**

Чтобы посмотреть, что у вас есть припрятанного, используйте: **git stash list**

Чтобы удалить спрятанное, выполните: **git stash drop stash\@{номер}**

Можно выполнить **git stash pop**, чтобы применить спрятанные изменения и сразу
же удалить их из стека.

Для скачивания репозитория используется команда:

**git pull**

Также выполняем ее, если не знаем были-ли внесены изменения на сервере. Чтобы, в
случае, если они были слить их себе на комп.
...
вначале перейдем в основную ветку в которую будем сливать другую:

**git checkout master**

**git merge dev** `#сливаем текущ. ветку master с веткой dev (подливаются ф-лы
из ветки dev), далее все изменения в ветке dev б. также вносится в ветку master`

**merge --abort** \#отменить merge
...
после **git reset** чтобы удалить все изменения созданные в версиях позже -
делаем **git checkout [имя_ф-ла]**
...
также эту команду м. использовать для создания новой ветки от активной в данный
момент (master):

**git checkout -b [имя_новой_ветки]** \#сразу переключаемся на новую ветку

`#каждая ветка имеет свои логи коммитов, при создании ветки в нее переносятся и
ф-лы исходной ветки`

**git branch**`#покажет существующие ветки`

**git checkout dev** `#переключимся на ветку dev`
...
**revert --continue** `#может использоваться для продолжения после разрешения
конфликтов`
